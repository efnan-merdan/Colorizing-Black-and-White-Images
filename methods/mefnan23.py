...
class GenerateMefnan23(DefaultColor):
    def __init__(self):
        super(GenerateMefnan23, self).__init__()
        ...
        model3 += [nn.ReLU(True), ]
        model3 += [nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=2, bias=True), ]
        # by changing paddin from 1 to 2, it gave the picture more warmth and natural realistic skin color
        model3 += [nn.ReLU(True), ]
        ...
        model6 = [nn.Conv2d(512, 512, kernel_size=3, dilation=2, stride=1, padding=1, bias=True), ]
        # by changing padding from 2 to 1, it gave the picture more soft features
        model6 += [nn.ReLU(True), ]
        model6 += [nn.Conv2d(512, 512, kernel_size=3, dilation=2, stride=1, padding=1, bias=True), ]
        # by changing paddin from 2 to 1, it gave the picture more shadow and reflective light
        model6 += [nn.ReLU(True), ]
        ...
        model8 += [nn.Conv2d(256, 313, kernel_size=1, stride=1, padding=1, bias=True), ]
        # by changing paddin from 0 to 1, it gave the background more definition and color
...
def mefnan23(pretrained=True):
    model = GenerateMefnan23()
    if (pretrained):
